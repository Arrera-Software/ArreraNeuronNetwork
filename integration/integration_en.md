...
Move the folders ```asset/```, ```brain/```, ```config/```, ```fnc/```, ```gestionnaire/```, ```gui/```, ```keyword/```, ```librairy/```, ```neuron/```, ```instruction_ia/``` and ```objet/``` to the root of your assistant project.
...
config = confNeuron(name="Opale",
                   ...
                   fichierKeyword="keyword/",
                    voiceAssistant=True)
```
The parameters starting with ```etat``` are parameters to be set to either 1 or 0, which corresponds to whether you want to activate a specific function of the assistant (including `etatMarkdown` for Markdown Server capabilities).
...
- 22 : Radio launch
- 23 : GUI Codehelp
- (Additional codes for Markdown/Server logic based on your implementation)
...
Assistants developed based on the Arrera Neuron Network can be connected to the Arrera application via a websocket or interact via Arrera Markdown.
